...
def hits_at_k_batch_torch(X_pred, heldout_batch, k=10):
    """
    hits@k for predictions [B, I] and ground-truth [B, I].
    """
    ...
    #true_item_nums =  ((X_true_binary ).sum(dim=1).float() > 0).float().sum()
    hit_vals = ((X_true_binary* X_pred_binary ).sum(dim=1).float() > 0).float()
